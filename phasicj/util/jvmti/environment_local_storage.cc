...
#include "phasicj/util/jvmti/environment_local_storage.h"
...
template<typename T>
void EnvironmentLocalStorage<T>::Set(jvmtiEnv& jvmti_env, unique_ptr<T> to_store) {
  SetPtr(jvmti_env, to_store.release());
}

template<typename T>
T& EnvironmentLocalStorage<T>::Get(jvmtiEnv& jvmti_env) {
  return *GetPtr(jvmti_env);
}

template<typename T>
void EnvironmentLocalStorage<T>::DeleteStored(jvmtiEnv& jvmti_env) {
  delete GetPtr(jvmti_env);
  SetPtr(jvmti_env, nullptr);
}

template<typename T>
T* EnvironmentLocalStorage<T>::GetPtr(jvmtiEnv& jvmti_env) {
  T* stored{};
...
template<typename T>
void EnvironmentLocalStorage<T>::SetPtr(jvmtiEnv& jvmti_env, T* to_store) {
  jint err {jvmti_env.SetEnvironmentLocalStorage(to_store)};
  switch (err) {
    case JVMTI_ERROR_NONE:
